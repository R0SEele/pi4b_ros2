...
# 这里改为 imu 可执行（你给我的代码）
CAR_CONTROLLER_CMD="ros2 run test_control_pkg imu --ros-args -p serial_port:=$CTRL_PORT -p enable_encoder_odom:=true -p enable_keyboard_control:=false -p send_odom_feedback:=false"
...
# else
    ...
    sleep 1
    launch_ros2_command_headless "ros2 launch bot_localization localization.launch.py" "ekf里程计融合" "$LOG_DIR/04_localization.log"
    ...
    if [ "$CAR_CONTROLLER_INTERACTIVE" = "1" ]; then
        echo "imu（car_controller）节点需要交互，改为当前终端前台启动。"
        echo "退出 imu 后，当前终端才会返回 shell。"
        sleep 1
