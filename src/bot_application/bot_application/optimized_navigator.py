...
class OptimizedWaypointNavigator(BasicNavigator):
    def __init__(self):
        ...
        # 路点
        self.waypoint_map = {
            1: self.create_goal_pose(1.3875, 0.11562, 1.5904),      # 西景点
            2: self.create_goal_pose(0.0562501, 0.0343748, 0.0322429), # 中景点
            3: self.create_goal_pose(-1.69999, 2.87813, -1.58023),    # 东景点
        }

        # ====================== 已更新：真实路点间路径点数（从你的日志替换） ======================
        self.cost_between = {
            1: {2: 240, 3: 224},   # 1→2=240点, 1→3=224点
            2: {1: 253, 3: 168},   # 2→1=253点, 2→3=168点
            3: {1: 174, 2: 118},   # 3→1=174点, 3→2=118点
        }
    ...
    def safe_get_path_len(self, start, goal, name):
        try:
            ...
            self.get_logger().error(f"✗ {name} 规划失败：返回空路径")
            return 0
    ...
    def run_once(self):
        ...
        # 计算路径长度
        p1 = self.safe_get_path_len(start_pose, wp1, "起点→路点1")
        p2 = self.safe_get_path_len(start_pose, wp2, "起点→路点2")
        p3 = self.safe_get_path_len(start_pose, wp3, "起点→路点3")
